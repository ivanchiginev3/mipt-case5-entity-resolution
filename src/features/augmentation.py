...
def augment_train_pairs(
    ...
    if len(positive_source) > 0:
        ...
        for _, row in sampled_pos.iterrows():
            ...
            new_row["name_latin_2"] = realistic_positive_augmentation(
                row["name_latin_2"]
            )
...
def remove_patronymic(text: str) -> str:
    ...
    # Для коротких имен ничего не удаляем
    if len(tokens) < 3:
        return text

    # Если 3 токена: фамилия имя отчество -> оставляем фамилия имя
    if len(tokens) == 3:
        return " ".join(tokens[:2])

    # Если 4+ токена, удаляем только последний токен,
    # а не обрезаем всё до первых двух
